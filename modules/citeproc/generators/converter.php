...
function convert_mods_to_citeproc_json_dates(SimpleXMLElement $mods) {
  $output = array();
  // Nigel's way
  $date = convert_mods_to_citeproc_json_query($mods, "//mods:originInfo/mods:dateCaptured[@encoding = 'iso8601']");
  if (!empty($date)) {
    ...
    $date = convert_mods_to_citeproc_json_query($mods, "//mods:originInfo/mods:dateCaptured");
    if(!empty($date)){
      $output['accessed']['raw'] = $date;
    }
  }
  $date = convert_mods_to_citeproc_json_query($mods, "//mods:originInfo/mods:dateIssued[@encoding = 'iso8601']");
  if (!empty($date)) {
    $date_time = new DateTime($date);
    $output['issued']['date-parts'] = array(array(intval($date_time->format('Y')), intval($date_time->format('m')), intval($date_time->format('d'))));
  }
  else {
    $date = convert_mods_to_citeproc_json_query($mods, "//mods:originInfo/mods:dateIssued");
    if(!empty($date)){
      $output['issued']['raw'] = $date;
    }
  }
  $date = convert_mods_to_citeproc_json_query($mods, "//mods:originInfo/mods:dateCreated[@encoding = 'iso8601']");
  if (!empty($date)) {
    $date_time = new DateTime($date);
    $output['issued']['date-parts'] = array(array(intval($date_time->format('Y')), intval($date_time->format('m')), intval($date_time->format('d'))));
  }
  else {
    $date = convert_mods_to_citeproc_json_query($mods, "//mods:originInfo/mods:dateCreated");
    if(!empty($date)){
      $output['issued']['raw'] = $date;
    }
  }
